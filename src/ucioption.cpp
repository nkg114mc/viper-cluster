...
void define_uci_options(void) {
  ...
  if(MaxNumOfThreads > 1) {
    ...
    define_uci_option("Minimum tree split depth", "4", UCI_SPIN, 3, 7);
  }
...
static void print_uci_option(uci_option_t *u) {
  ...
  printf("option name %s ", u->name);
  printf("type %s ", option_type_name[u->type]);
  if(u->type != UCI_BUTTON) printf("default %s ", u->default_value);
  switch(u->type) {
  case UCI_SPIN: printf("min %d max %d", u->min, u->max); break;
  case UCI_COMBO:
    for(i = 0; strlen(u->combo_vars[i]) > 0; i++)
      printf("var %s ", u->combo_vars[i]);
    break;
  default: break;
  }
  printf("\n");
